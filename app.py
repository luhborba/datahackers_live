...
def main():
    ...
    with st.expander("🛒 Registrar Venda:"):
        with st.form("Registro de Venda"):
            produto = st.selectbox('Selecione o Produto', ['TV', 'Notebook', 'Celular', 'Tablet', 'PC', 'Geladeira', 'Fogão', 'Microondas'], index=None, placeholder='Escolha o Produto')
            categoria = st.selectbox('Selecione a Categoria', ['Eletrodomesticos', 'Informática'], index=None, placeholder='Escolha a Categoria')
            preco = st.number_input('Preço R$', value=0.0)
            quantidade = st.number_input('Quantidade', value=0)
            forma_pagamento = st.selectbox('Forma de Pagamento', ['Dinheiro', 'Cartão de Crédito', 'Cartão de Debito', 'Boleto','Pix'], index=None, placeholder='Escolha a Forma de Pagamento')
            salvar = st.form_submit_button("Registrar Venda")

            if salvar:
                try:
                    df = carregar_dados()
                    novo_registro = pd.DataFrame([[produto, categoria, preco, quantidade, forma_pagamento, data_atual]], columns=['Produtos', 'Categorias', 'Preço', 'Quantidade', 'Forma de Pagamento', 'DataVenda'])
                    df_final = pd.concat([df, novo_registro])
                    df_final.to_csv('data/vendas.csv', index=False)
                except:
                    df = pd.DataFrame([[produto, categoria, preco, quantidade, forma_pagamento, data_atual]], columns=['Produtos', 'Categorias', 'Preço', 'Quantidade', 'Forma de Pagamento', 'DataVenda'])
